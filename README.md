...
into `src/vendor/` → download base weights → validate). Subsequent runs skip
straight to starting the web UI at `http://127.0.0.1:7860`.
...
└── src/
    ...
    ├── data/          references/, models/, outputs/ generated at runtime
    ├── vendor/        RVC and Fish Speech repos, cloned by the installer
...
`tools/synthesize_cli.py`) inside the vendored repos under `src/vendor/`. These
